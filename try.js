...
class UI {
  static TOKEN_MAP = Object.freeze({
    "{text}": "\x1b[39m",
    "{dim_text}": "\x1b[90m",
    "{h1}": "\x1b[1;33m",
    "{h2}": "\x1b[1;36m",
  ...
  print(text, io = process.stderr) {
    if (text == null) return;
    this.currentLine += text;
  }

  puts(text = "", io = process.stderr) {
    this.currentLine += text;
    this.buffer.push(this.currentLine);
    this.currentLine = "";
  }

  cls(io = process.stderr) {
    this.currentLine = "";
    this.buffer = [];
    this.lastBuffer = [];
...
class TrySelector {
  ...
  calculateScore(text, query, ctime, mtime) {
    let score = 0.0;
  ...
  highlightMatchesForSelection(text, query) {
    if (!query || query.length === 0) return text;
    let result = "";
    const textLower = text.toLowerCase();
    const queryLower = query.toLowerCase();
...
class ArgsParser {
  ...
  static extractOptionWithValue(args, optName) {
    const i = [...args]
      .reverse()
...
class GitUtils {
  ...
  static generateCloneDirectoryName(gitUri, customName) {
...
class PathUtils {
  static uniqueDirName(triesPath, dirName) {
  ...
  static resolveUniqueNameWithVersioning(triesPath, datePrefix, base) {
...
class InitScript {
  static bashOrZsh(scriptPath, pathArg) {
    return `try() {
...
`;
  ...
  static fish(scriptPath, pathArg) {
    return `function try
...
class TaskScriptEmitter {
  static joinCommands(parts) {
    return parts.join(" \\\n  && ");
  }

  static emitScript(parts) {
    process.stdout.write(TaskScriptEmitter.joinCommands(parts));
  }

  static emitTasksScript(tasks) {
...
class Commands {
  constructor(triesPath, debugEnabled) {
  ...
  worktree(args) {
    ...
    if (sub == null || sub === "dir") {
      ...
      const fullPath = path.join(this.triesPath, dirName);
      const tasks = [{ type: "target", path: fullPath }, { type: "mkdir" }];
  ...
  cd(args, andType, andExit, andKeys, andConfirm) {
    Debug.log(
      this,
      "cd_command",
    ...
    return selector.run().then((result) => {
      ...
      const tasks = [{ type: "target", path: result.path }];
